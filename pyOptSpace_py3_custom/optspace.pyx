...
def optspace(sparse_mat, rank_n, num_iter, tol, verbosity, outfile):
    ...
    Returns
    =======
    [X,S,Y]: each is a matrix, so that X * S * Y.T will be the completed matrix
    '''
    ...
    recon_var = OptSpace(M, rank_n, num_iter, tol, verbosity, outfile.encode())
    ...
    return (X_np, S_np, Y_np, recon_var.out_niter)
